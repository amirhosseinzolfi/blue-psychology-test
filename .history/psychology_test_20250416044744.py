...
def get_available_tests():
    """Returns a list of available test names and their indices."""
    return [{"name": test['test_name'], "index": idx} for idx, test in enumerate(all_tests["tests"])]
    
def tele_initialize(user_name: str, test_index: int = 0):
    """Initializes the test state for Telegram, selecting the specified test."""
    global test_data
    # Select the test based on the provided index
    if 0 <= test_index < len(all_tests["tests"]):
        test_data = all_tests["tests"][test_index]
        log.info(f"Telegram: Initializing test '{test_data['test_name']}' for user {user_name}")
    else:
        # Fallback to the first test if index is invalid
        log.warning(f"Invalid test index {test_index} provided. Falling back to the first test.")
        test_data = all_tests["tests"][0]
        
    # Clear previous results for the new test session
    global test_results
    test_results = {
        "test_name": test_data.get('test_name', ''),
        "answers": []
    ...
    state = {
        ...
        "conversation_history": conversation_history,
        "last_answer": None,
        "test_index": test_index # Store the index for potential future use
    }
    return state
