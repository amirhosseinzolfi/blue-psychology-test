...
def start(update: Update, context: CallbackContext) -> None:
    # Use Persian labels without leading slash in the reply keyboard.
    keyboard = [[ "آزمون روانشناسی", "آزمون شخصیتی" ]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True)
    update.message.reply_text(
        "خوش آمدید به ربات آزمون روانشناسی.\nلطفاً یکی از آزمون‌های موجود را انتخاب کنید:",
...
def start_test(update: Update, context: CallbackContext) -> None:
    ...
    test_type = None
    # Check if the message indicates a specific test type.
    if update.message.text == "آزمون روانشناسی":
        test_type = "A"
    elif update.message.text == "آزمون شخصیتی":
        test_type = "B"
    ...
    update.message.reply_text(f"سلام {user_name}، آزمون شما آغاز می‌شود:\n\n{q_text}")
...
def main():
    ...
    # Remove Persian CommandHandlers because they cause errors.
    # Add a MessageHandler to catch Persian test selection texts.
    dispatcher.add_handler(MessageHandler(Filters.regex("^(آزمون روانشناسی|آزمون شخصیتی)$"), start_test))
