...
def tele_initialize(user_name: str, test_type: str = None):
    # Select the test based on test_type or default to the first test
    global test_data
    selected_test_index = 0  # Default to the first test
    if test_type:
        # Assuming test_type 'A' maps to index 0, 'B' to index 1, etc.
        # Adjust this logic if test types have different identifiers
        if test_type.upper() == 'A' and len(all_tests["tests"]) > 0:
            selected_test_index = 0
        elif test_type.upper() == 'B' and len(all_tests["tests"]) > 1:
            selected_test_index = 1
        # Add more conditions if there are more tests
        else:
            log.warning(f"Invalid or unknown test_type '{test_type}', defaulting to the first test.")

    if selected_test_index < len(all_tests["tests"]):
        test_data = all_tests["tests"][selected_test_index]
        log.info(f"Telegram: Initializing test '{test_data['test_name']}' for user {user_name}")
    else:
        log.error("No tests available or index out of bounds. Cannot initialize test.")
        # Handle error state appropriately, maybe return None or raise exception
    ...
    state = {
        ...
        "conversation_history": conversation_history,
        "last_answer": None,
        "test_name": test_data.get('test_name', 'Unknown Test') # Store test name in state
    }
    # Reset test_results for the new session
    global test_results
    test_results = {
        "test_name": test_data.get('test_name', ''),
        "answers": []
