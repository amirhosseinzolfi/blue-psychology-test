...
def login(update: Update, context: CallbackContext) -> None:
    update.message.reply_text("در حال ورود از طریق ترمینال. لطفاً ترمینال را بررسی کنید.")
...
def test_message_handler(update: Update, context: CallbackContext) -> None:
    ...
    if user_id in user_tests:
        ...
        update.message.reply_text("برای شروع آزمون، دستور /start_test را تایپ کنید.")
...
def message_handler(update: Update, context: CallbackContext) -> None:
    ...
    if user_id not in user_tests:
        ...
        response = f"شما گفتید: {user_text}"
